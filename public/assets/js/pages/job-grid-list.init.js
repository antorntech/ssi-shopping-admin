    ...
    e[s] &&
      ...
      Array.from(e[s].requirement).forEach(function (e, t) {
        ...
        "Full Time" == e
          ...
          (n += '<span className="badge rounded-1 ' + a + '">' + e + "</span>");
      }),
      (document.querySelector("#jobgrid-list").innerHTML +=
        '<div className="col-xl-3 col-md-6">        <div className="card">            <div className="card-body">                <img src="' +
        e[s].image +
        '" alt="" height="50" className="mb-3">                <h5 className="fs-17 mb-2"><a href="#;" className="text-dark">' +
        e[s].jobTitle +
        '</a> <small className="text-muted fw-normal">(' +
        e[s].experience +
        ')</small></h5>                <ul className="list-inline mb-0">                    <li className="list-inline-item">                        <p className="text-muted fs-14 mb-1">' +
        e[s].companyName +
        '</p>                    </li>                    <li className="list-inline-item">                        <p className="text-muted fs-14 mb-0"><i className="mdi mdi-map-marker"></i>' +
        e[s].location +
        '</p>                    </li>                    <li className="list-inline-item">                        <p className="text-muted fs-14 mb-0"><i className="uil uil-wallet"></i>' +
        e[s].salary +
        '</p>                    </li>                </ul>                <div className="mt-3 hstack gap-2">' +
        n +
        '</div>                <div className="mt-4 hstack gap-2">                    <a href="#!" data-bs-toggle="modal" className="btn btn-soft-success w-100">View Profile</a>                    <a href="#applyJobs" data-bs-toggle="modal" className="btn btn-soft-primary w-100">Apply Now</a>                </div>            </div>        </div>    </div>'));
  }
  selectedPage(),
    1 == currentPage
...
function searchResult(e) {
  ...
  for (var a = Math.ceil(e.length / itemsPerPage), n = 1; n < a + 1; n++)
    t.innerHTML +=
      "<div class='page-item'><a class='page-link clickPageNumber' href='#;'>" +
      n +
      "</a></div>";
}
